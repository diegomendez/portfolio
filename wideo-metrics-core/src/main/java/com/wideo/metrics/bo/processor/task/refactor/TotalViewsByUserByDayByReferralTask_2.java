...
public class TotalViewsByUserByDayByReferralTask_2 extends Task_2 {

    public TotalViewsByUserByDayByReferralTask_2(DBCollection dbCollection, StatsFilterProcessorBOImpl statsFilterProcessorBOImpl) {
    ...
    public TaskResponse process(ProcessingKey processingKey,
        ...
        try {
            ...
            sbMapKey.append(",");
            sbMapKey.append("referral: this.referral");
            sbMapKey.append("}");
            ...
            sbMap.append("emit(key, {user_id: this.user_id, date: date_utc, action: this.action, referral: this.referral, count: 1});");
            sbMap.append("}");
            ...
            String reduce = "function (key, values) { "
                    ...
                    + " } "
                    + " return { user_id: values[i].user_id, date: values[i].date, action: values[i].action, referral: values[i].referral, count:total} }";
            ...
            try {
                ...
                filterUserStatsByProperty(out.results(), (Date) dateObject.get("$gte"), "os");
            }
