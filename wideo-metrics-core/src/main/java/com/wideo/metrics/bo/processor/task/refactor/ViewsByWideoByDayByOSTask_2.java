...
public class ViewsByWideoByDayByOSTask_2 extends Task_2 {

    public ViewsByWideoByDayByOSTask_2(DBCollection dbCollection, StatsFilterProcessorBOImpl statsFilterProcessorBOImpl) {
    ...
    public TaskResponse process(ProcessingKey processingKey,
        ...
        try {
            ...
            sbMap.append("emit(key, { wideo_id: this.wideo_id, date: date_utc, action: this.action, os: this.os, count: 1});");
            sbMap.append("}");
            ...
            String reduce = "function (key, values) { "
                    ...
                    + " } "
                    + " return { wideo_id: values[i].wideo_id, date: values[i].date, action: values[i].action, os: values[i].os, count:total } }";
            ...
            try {
                ...
                filterStatsByProperty(out.results(), (Date) dateObject.get("$gte"), "os");
            }
