...
public class TotalViewsByUserByDayByDeviceTask_2 extends Task_2 {

    public TotalViewsByUserByDayByDeviceTask_2(DBCollection dbCollection, StatsFilterProcessorBOImpl statsFilterProcessorBOImpl) {
    ...
    public TaskResponse process(ProcessingKey processingKey,
        ...
        try {
            ...
            sbMap.append("emit(key, {user_id: this.user_id, date: date_utc, action: this.action, device_category: this.device_category, count: 1});");
            sbMap.append("}");
            ...
            String reduce = "function (key, values) { "
                    ...
                    + " } "
                    + " return { user_id: values[i].user_id, date: values[i].date, action: values[i].action, device_category: values[i].device_category, count:total} }";
            ...
            try {
                ...
                filterUserStatsByProperty(out.results(), (Date) dateObject.get("$gte"), "device_category");
            }
