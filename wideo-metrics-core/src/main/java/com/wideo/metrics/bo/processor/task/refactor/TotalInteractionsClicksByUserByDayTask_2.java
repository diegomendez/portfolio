...
public class TotalInteractionsClicksByUserByDayTask_2 extends Task_2 {

    public TotalInteractionsClicksByUserByDayTask_2(DBCollection dbCollection, StatsFilterProcessorBOImpl statsFilterProcessorBOImpl) {
    ...
    public TaskResponse process(ProcessingKey processingKey,
        ...
        try {
            ...
            sbMapKey.append("\""
                    + WideoEventTypesEnum.OBJECT_CLICK.getName()
                    + "\"");
            sbMapKey.append(",");
            ...
            sbMap.append("emit(key, { user_id: this.user_id, date: date_utc, action: this.action, count: 1});");
            sbMap.append("}");
            ...
            String reduce = "function (key, values) { "
                    ...
                    + " } "
                    + " return { user_id: values[i].user_id, date: values[i].date, action: values[i].action, count:total } }";
            ...
            try {
                ...
                filterUserStats(out.results(), (Date) dateObject.get("$gte"));
            }
