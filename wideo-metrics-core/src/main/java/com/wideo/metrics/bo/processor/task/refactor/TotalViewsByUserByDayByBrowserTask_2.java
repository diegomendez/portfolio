...
public class TotalViewsByUserByDayByBrowserTask_2 extends Task_2 {

    public TotalViewsByUserByDayByBrowserTask_2(DBCollection dbCollection, StatsFilterProcessorBOImpl statsFilterProcessorBOImpl) {
    ...
    public TaskResponse process(ProcessingKey processingKey,
        ...
        try {
            ...
            sbMapKey.append(",");
            sbMapKey.append("browser: this.browser");
            sbMapKey.append("}");
            ...
            sbMap.append("emit(key, {user_id: this.user_id, date: date_utc, action: this.action, browser: this.browser, count: 1});");
            sbMap.append("}");
            ...
            String reduce = "function (key, values) { "
                    ...
                    + " } "
                    + " return { user_id: values[i].user_id, date: values[i].date, action: values[i].action, browser: values[i].browser, count:total} }";
            ...
            try {
                ...
                filterUserStatsByProperty(out.results(), (Date) dateObject.get("$gte"), "browser");
            }
