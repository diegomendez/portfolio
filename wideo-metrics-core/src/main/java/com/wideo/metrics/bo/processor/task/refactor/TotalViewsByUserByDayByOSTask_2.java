...
public class TotalViewsByUserByDayByOSTask_2 extends Task_2 {

    public TotalViewsByUserByDayByOSTask_2(DBCollection dbCollection, StatsFilterProcessorBOImpl statsFilterProcessorBOImpl) {
    ...
    public TaskResponse process(ProcessingKey processingKey,
        ...
        try {
            ...
            sbMap.append("emit(key, {user_id: this.user_id, date: date_utc, action: this.action, os: this.os, count: 1});");
            sbMap.append("}");
            ...
            String reduce = "function (key, values) { "
                    ...
                    + " } "
                    + " return { user_id: values[i].user_id, date: values[i].date, action: values[i].action, os: values[i].os, count:total} }";
